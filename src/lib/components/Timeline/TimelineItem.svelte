...
<li class="mb-10 ml-10">
	<!-- Slot: Icon -->
	<span
		class="absolute flex items-center justify-center w-6 h-6 bg-secondary-300 text-secondary-900 font-bold rounded-full -left-3 ring-8 ring-surface-200-700-token"
	>
	...
	<h3 class="flex items-center mb-1 space-x-2 -translate-y-2">
		...
		<!-- Badge -->
		{#if badgeText}
			<span class="badge variant-soft-surface">{badgeText}</span>
		{/if}
	</h3>
	<!-- Timestamp -->
